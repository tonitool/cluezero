...
import { getCampaignPerformanceComposio } from '@/lib/google-ads'
import { getMetaCampaignPerformanceComposio } from '@/lib/meta-ads'
  ...
  for (const conn of conns) {
    try {
      if (conn.app_name === 'googleads') {
        ...
        const campaigns = await getCampaignPerformanceComposio(workspaceId, customerId, 30)
        ...
        const campaigns = await getMetaCampaignPerformanceComposio(workspaceId, accountId, 30)
        const spend = campaigns.reduce((s, c) => s + c.spend, 0)
        totalSpend       += spend
        totalClicks      += campaigns.reduce((s, c) => s + c.clicks, 0)
        totalConversions += campaigns.reduce((s, c) => s + c.conversions, 0)
