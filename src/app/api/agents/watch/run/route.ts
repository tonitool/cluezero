...
import { z } from 'zod'
import { createAsanaTask, refreshAsanaToken } from '@/lib/asana'
import { createClickUpTask, severityToClickUpPriority } from '@/lib/clickup'
...
async function fireAsanaTasks(admin: Admin, workspaceId: string, output: AgentOutput): Promise<ActionTaken> {
  const { data: conn } = await admin.from('productivity_connections').select('*').eq('workspace_id', workspaceId).eq('platform', 'asana').single()
  ...
  for (const f of highs) {
    const task = await createAsanaTask(accessToken, {
...
async function fireClickUpTasks(admin: Admin, workspaceId: string, output: AgentOutput): Promise<ActionTaken> {
  const { data: conn } = await admin.from('productivity_connections').select('*').eq('workspace_id', workspaceId).eq('platform', 'clickup').single()
  ...
  let created = 0
  for (const f of highs) {
    const task = await createClickUpTask(conn.access_token, {
