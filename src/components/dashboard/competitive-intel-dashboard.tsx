...
export function CompetitiveIntelDashboard({
  ...
  const checkSyncStatus = useCallback(() => {
    ...
    fetch(`/api/connections/list?workspaceId=${workspaceId}`)
      .then(r => r.json())
      .then(d => {
        const sfConns = (d.connections ?? []).filter((c: { app_name: string }) => c.app_name === 'snowflake')
        if (sfConns.length > 0) {
          setSources(sfConns.map((c: { id: string; name: string }) => ({ id: c.id, name: c.name })))
          setSyncingGlobal(sfConns.some((c: { sync_status: string }) => c.sync_status === 'syncing'))
        } else {
